...
class Constancia {
    ...
    public:
        ...
        bool operator == (const Constancia& x) {
            if (nombre == x.nombre ) {
                if (carrera == x.carrera) {
                    if (totalMaterias == x.totalMaterias) {
                        if (promedio == x.promedio) {
                            return true;
                        }
                    }
                }
...
template<class DATO>
int Cola <DATO>::busqueda(DATO buscado) {
...
int main () {
    ...
    do {
        ...
        cout << "4) Buscar una constancia." <<endl;
        ...
        switch (opc) {
            ...
            case 4:{
                ...
                if (estudiantes.vacia()) {
                    ...
                    system(CLEAR);
                    cout << "***DIRECCION ESCOLAR***" <<endl;
                    cout << "***BUSCAR UNA CONSTANCIA***" <<endl<<endl;

                    cout << "No hay datos que buscar..."; pausa();
                    ...
                    limpiaBuffer();
                    getline(cin, auxNombre);
                    cout << "Dame la carrera que estudia: ";
                    fflush(stdin);
                    getline(cin, auxCarrera);
                    cout << "Dame el total de materias del alumno: ";
                    cin >> auxInt;
                    cout << "Dame el promedio del alumno: ";
                    cin >> auxFloat;
                    
                    aux.setNombre(auxNombre);
                    aux.setCarrera(auxCarrera);
                    aux.setTotalMaterias(auxInt);
                    aux.setPromedio(auxFloat);

                    int indiceBuscado = estudiantes.busqueda(aux);

                    cout << endl << "Indice del buscado: " << indiceBuscado + 1 <<endl;
                    cout << "Numero de constancias a elaborar para llegar a esta: " << indiceBuscado <<endl;
                    cout << "Nota: Si el indice es -1, significa que no existe en el registro.";
                    pausa();
